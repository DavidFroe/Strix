...
/// Resolve a model name argument from `/model`/`/submodel`. Accepts:
///   1. canonical DeepSeek IDs (`deepseek-v4-pro`) and their aliases
///      (`owltrail-pro`, `deepseek-v4pro`), via [`normalize_model_name`];
///   2. any model ID the adapter advertised in `app.available_models` —
///      this is what makes `/submodel free` or `/model claude-haiku-4-5`
///      work even though they're not DeepSeek-flavoured.
///
/// Returns `Some(id)` with the resolved (possibly canonicalized) ID, or
/// `None` when the name doesn't match any known shape.
fn resolve_picker_model_arg(app: &App, name: &str) -> Option<String> {
    ...
    // Adapter-advertised IDs pass through unchanged. Match case-insensitively
    // so users don't have to mirror the exact capitalisation.
    if app
        .available_models
        .iter()
        .any(|m| m.id.eq_ignore_ascii_case(&lower))
    {
        // Use the canonical adapter casing.
        return app
            .available_models
            .iter()
...
pub fn model(app: &mut App, model_name: Option<&str>) -> CommandResult {
    if let Some(name) = model_name {
        ...
        let Some(model_id) = resolve_picker_model_arg(app, name) else {
            return CommandResult::error(format!(
                "Unknown model '{name}'. Use auto, an OwlTrail model ID from /models, or one of: {}",
...
/// Switch or view the sub-agent model. With no argument, open the picker.
/// `inherit` / `none` / `default` resets the override so sub-agents reuse
/// the main model again.
pub fn submodel(app: &mut App, model_name: Option<&str>) -> CommandResult {
    if let Some(name) = model_name {
        let trimmed = name.trim();
        if trimmed.is_empty()
            || matches!(
                trimmed.to_ascii_lowercase().as_str(),
                "inherit" | "none" | "default" | "(default)" | "off"
            )
        {
            app.subagent_model = None;
            return CommandResult::with_message_and_action(
                "Sub-agent model cleared (inherits main model).".to_string(),
                AppAction::ApplySubagentModel { model: None },
            );
        }
        let Some(model_id) = resolve_picker_model_arg(app, trimmed) else {
            return CommandResult::error(format!(
                "Unknown sub-agent model '{name}'. Use inherit, an OwlTrail model ID from /models, or one of: {}",
                COMMON_DEEPSEEK_MODELS.join(", ")
            ));
        };
        app.subagent_model = Some(model_id.clone());
        let display = crate::models::display_model_name(&model_id);
        CommandResult::with_message_and_action(
            format!("Sub-agent model set to: {display}"),
            AppAction::ApplySubagentModel {
                model: Some(model_id),
            },
        )
    } else {
        CommandResult::action(AppAction::OpenSubmodelPicker)
    }
}
...
mod tests {
    ...
    fn test_home_dashboard_includes_all_sections() {
        ...
        assert!(result.message.is_some());
        let msg = result.message.unwrap();
        assert!(msg.contains("Propeller Home Dashboard"));
