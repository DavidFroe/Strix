...
pub const COMMANDS: &[CommandInfo] = &[
    ...
    CommandInfo {
        name: "submodel",
        aliases: &["sub-model", "agent-model", "subagent-model"],
        usage: "/submodel [name|inherit]",
        description_id: MessageId::CmdSubmodelDescription,
    },
    CommandInfo {
        name: "provider",
        aliases: &[],
    ...
    // Vibecoding-Kontext-Files (Read-only Anzeige der MDs im Arbeitsverzeichnis).
    // Descriptions teilen sich CmdContextDescription — die Commands sind trivial
    // und brauchen keine eigene Lokalisierung.
    CommandInfo {
        name: "spec",
        aliases: &["plan"],
        usage: "/spec",
...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    match command {
        ...
        "submodel" | "sub-model" | "agent-model" | "subagent-model" => {
            core::submodel(app, arg)
        }
        ...
        // "context" liefert jetzt die Vibecoding-Datei-Übersicht; "ctx" bleibt
        // der alte Context-Window-Stats-Befehl für Tokens/Cache-Diagnose.
        "context" => context_files::context(app),
        "ctx" => debug::context(app),
        "spec" | "plan" => context_files::spec(app),
        "tagebuch" | "diary" | "log" => context_files::tagebuch(app, arg),
        "claude" => context_files::claude(app),
        "edit" => debug::edit(app),
        "diff" => debug::diff(app),
        "undo" => {
