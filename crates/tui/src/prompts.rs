...
pub struct PromptSessionContext<'a> {
    ...
    /// Resolved BCP-47 locale tag for the `## Environment` block.
    pub locale_tag: &'a str,
    /// Active model ID shown in `## Environment` so the model knows which
    /// variant it's running as (e.g. `"deepseek-v4-pro"` vs `"deepseek-v4-flash"`).
    /// Pass `""` when the model is not yet known.
    pub model: &'a str,
}
...
pub fn system_prompt_for_mode_with_context_skills_session_and_approval(
    ...
    full_prompt = format!(
        "{full_prompt}\n\n{}",
        render_environment_block(workspace, session_context.locale_tag, session_context.model),
    );
    ...
    // 4. Context Management (Agent / Yolo / Propeller only).
    if matches!(mode, AppMode::Agent | AppMode::Yolo | AppMode::Propeller) {
