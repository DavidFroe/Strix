...
const DEFAULT_STREAM_OPEN_TIMEOUT: Duration = Duration::from_secs(120);
...
impl DeepSeekClient {
    pub(super) async fn handle_chat_completion_stream(
        ...
        let stream = async_stream::stream! {
            ...
            let mut in_content_think = false;
...
/// Split `content` into alternating (is_think, text) parts by tracking
/// `<think>...</think>` tag boundaries across chunk calls via `in_think`.
fn split_think_content(content: &str, in_think: &mut bool) -> Vec<(bool, String)> {
    ...
    loop {
        if *in_think {
            match remaining.find("</think>") {
                Some(pos) => {
                    if pos > 0 {
                        parts.push((true, remaining[..pos].to_string()));
                    }
                    *in_think = false;
                ...
                None => {
                    if !remaining.is_empty() {
                        parts.push((true, remaining.to_string()));
                    }
                    break;
                }
            }
            ...
            match remaining.find("<think>") {
                Some(pos) => {
                    if pos > 0 {
                        parts.push((false, remaining[..pos].to_string()));
                    }
                    *in_think = true;
                    remaining = &remaining[pos + "<think>".len()..];
                }
                None => {
                    if !remaining.is_empty() {
                        parts.push((false, remaining.to_string()));
                    }
                    break;
                }
            }
...
pub(super) fn parse_sse_chunk(
    ...
    for choice in choices {
        ...
        if let Some(delta) = delta {
            ...
            // Handle regular content — split out <think>...</think> blocks that
            // Qwen3/Ollama embed directly in the content field and route them as
            // native ThinkingDelta events so they appear as a collapsible block.
            if let Some(content) = delta.get("content").and_then(Value::as_str)
                && !content.is_empty()
            {
                for (is_think, part) in split_think_content(content, in_content_think) {
                    if part.is_empty() {
                        continue;
                    }
                    if is_think {
                        ...
                        events.push(StreamEvent::ContentBlockDelta {
                            index: *content_index,
                            delta: Delta::ThinkingDelta { thinking: part },
                        });
                    } else {
                        if *thinking_started {
                            events.push(StreamEvent::ContentBlockStop {
                        ...
                        events.push(StreamEvent::ContentBlockDelta {
                            index: *content_index,
                            delta: Delta::TextDelta { text: part },
                        });
                    }
                }
            }
