...
pub enum Op {
    ...
    /// Update the model used as the base for sub-agent / background-task
    /// spawns. `None` clears the override so sub-agents inherit the main
    /// model again.
    #[allow(dead_code)]
    SetSubagentModel { model: Option<String> },
