...
pub struct Session {
    ...
    /// Optional model override used as the base model when spawning sub-agents
    /// / background tasks. `None` means children inherit `model`. Persisted
    /// in `Settings.subagent_model` and synced from the TUI via
    /// `Op::SetSubagentModel`.
