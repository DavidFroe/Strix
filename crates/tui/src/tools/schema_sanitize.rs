...
/// Sanitize a JSON Schema in-place for strict-tool compatibility.
///
/// Applies a sequence of normalisations chosen to be semantics-preserving:
/// - Collapse `{"anyOf":[X, {"type":"null"}]}` → `X ∪ {"nullable": true}`
/// - Strip `anyOf`/`oneOf`/`allOf` whose every branch is `{"required":[…]}`
///   (validation-only union — Claude and DeepSeek reject these at any level)
...
pub fn sanitize(schema: &mut Value) {
    ...
    strip_required_only_unions(schema);
...
/// Remove `anyOf`/`oneOf`/`allOf` unions where every branch is solely a
/// `{"required": […]}` object.  These are validation-only constraints that
/// express "one of these fields must be present"; they carry no type
/// information and are rejected by the Claude API (and DeepSeek strict mode)
/// at any schema level.  Dropping them makes the parameters fully optional in
/// the JSON Schema sense — the model's field descriptions already guide the LLM
/// to supply the right one.
fn strip_required_only_unions(schema: &mut Value) {
    let Some(obj) = schema.as_object_mut() else {
        return;
    };
    for key in ["anyOf", "oneOf", "allOf"] {
        if let Some(Value::Array(arr)) = obj.get(key) {
            let all_required_only = arr.iter().all(|alt| {
                alt.as_object()
