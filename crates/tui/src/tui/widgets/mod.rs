...
pub use footer::{
    FooterProps, FooterToast, FooterWidget, footer_agents_chip,
};
    ...
    fn render(&self, _area: Rect, buf: &mut Buffer) {
        ...
        if let Some(scrollbar) = self.scrollbar {
            ...
            Scrollbar::new(ScrollbarOrientation::VerticalRight)
                ...
                .end_symbol(None)
                .track_symbol(Some("│"))
                .track_style(Style::default().fg(palette::accent_border()))
                .thumb_symbol("┃")
                .thumb_style(Style::default().fg(palette::accent_light()))
...
impl<'a> ComposerWidget<'a> {
    ...
    /// True when the "/" hint is shown in the composer bottom border.
    pub fn slash_hint_visible(&self, area: Rect) -> bool {
        self.has_panel(area)
            && self.app.composer_display_input().trim().is_empty()
...
impl Renderable for ComposerWidget<'_> {
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if has_panel {
            ...
            if self.app.composer.vim_enabled {
                let color = match self.app.composer.vim_mode {
                    ...
                    VimMode::Insert => palette::accent_light(),
        ...
        let placeholder_owned: String = if self.app.is_history_search_active() {
            self.app
                .tr(crate::localization::MessageId::HistorySearchPlaceholder)
                .to_owned()
        } else {
            self.app.composer_placeholder_text().to_owned()
        };
        ...
        let visual_rows = if input_text.is_empty() {
            placeholder_visual_lines_for(placeholder_owned.as_str(), content_width)
        } else {
            input_lines.len()
        };
...
fn category_label_for(category: ToolCategory, locale: Locale) -> (&'static str, Color) {
    match (locale, category) {
        ...
        (Locale::ZhHans, ToolCategory::McpRead) => ("MCP 读取", palette::accent_light()),
        ...
        (_, ToolCategory::McpRead) => ("MCP Read", palette::accent_light()),
...
fn build_empty_state_lines(app: &App, area: Rect) -> Vec<Line<'static>> {
    ...
    let body_width = usize::from(area.width.saturating_sub(8).clamp(24, 80));
    let left_padding = usize::from(area.width.saturating_sub(body_width as u16) / 2);
    let inset = " ".repeat(left_padding);

    let body: Vec<Line<'static>> = if let Some(_splash) = &app.splash {
        // Splash still running — show loading hint while inference is pending.
        vec![
            Line::from(Span::styled(
                format!("{inset}\u{22ef}  Starte Propeller \u{22ef}"),
                Style::default().fg(palette::TEXT_MUTED),
            )),
        ]
    } else if let Some((quote, attribution, greeting)) = app.startup_quote.as_ref() {
        // Inference succeeded — show quote + greeting, word-wrapped to available width.
        let mut lines: Vec<Line<'static>> = Vec::new();

        // Quote — wrap to full body width.
        for wrapped in wrap_to_width(quote, body_width) {
            lines.push(Line::from(Span::styled(
        ...
        // Attribution — first line prefixed with "  — ", continuation indented.
        let attr_prefix = "  \u{2014} ";
        let attr_prefix_width: usize = 4;
        for (i, wrapped) in wrap_to_width(attribution, body_width.saturating_sub(attr_prefix_width)).iter().enumerate() {
            let prefix = if i == 0 { attr_prefix } else { "    " };
            lines.push(Line::from(Span::styled(
                format!("{inset}{prefix}{wrapped}"),
        ...
        // No quote yet or test was skipped — show a neutral prompt.
        vec![
            Line::from(Span::styled(
                format!("{inset}\u{1F300}  Propeller"),
...
mod tests {
    ...
    fn selection_style_uses_explicit_selection_text_role() {
        ...
        assert_eq!(styled[0].style.bg, Some(palette::accent_selection()));
