...
/// Glyph palette for the working-strip animation, dark→bright.
pub(crate) const PROP_GLYPHS: [char; 5] = [
    ' ',
    '\u{2591}', // ░
    '\u{2592}', // ▒
    '\u{2593}', // ▓
...
/// One frame of the footer working-strip animation.
///
/// A single Gaussian brightness spot glides smoothly left→right→left
/// (cosine ping-pong, period 3 s). Clean single blob, no overlapping blades.
#[must_use]
pub fn footer_working_strip_glyph_at(col: usize, width: usize, frame: u64) -> char {
    ...
    // Cosine ping-pong: 0..1..0 over 3 s, maps to left..right..left
    let period = 3.0_f64;
    let pos = (std::f64::consts::PI * t / period).cos();  // -1..1
    let center = (1.0 - pos) / 2.0 * (width - 1) as f64; // 0..width-1
    let sigma = (width as f64 * 0.12).max(2.0);
    let dist = col as f64 - center;
    let brightness = (-0.5 * (dist / sigma).powi(2)).exp();

    let idx = (brightness * (PROP_GLYPHS.len() - 1) as f64).round() as usize;
    PROP_GLYPHS[idx.min(PROP_GLYPHS.len() - 1)]
}
...
fn mode_style(app: &App) -> (&'static str, Color) {
    let label = match app.mode {
        ...
        // AppMode::Propeller bleibt (Refactor wäre 30+ Stellen).
        AppMode::Propeller => "auto",
    };
    let color = match app.mode {
        AppMode::Chat => app.ui_theme.mode_chat,
        AppMode::Plan => app.ui_theme.mode_plan,
        AppMode::Agent => app.ui_theme.mode_agent,
        AppMode::Shell => app.ui_theme.mode_shell,
        AppMode::Yolo => app.ui_theme.mode_yolo,
        AppMode::Propeller => app.ui_theme.mode_propeller,
    };
    (label, color)
}
...
mod tests {
    ...
    fn working_strip_renders_glyphs_only_when_frame_is_some() {
        ...
        assert!(
            active
                .chars()
                .any(|glyph| super::PROP_GLYPHS.contains(&glyph) && glyph != ' '),
    ...
    fn working_strip_renders_multiple_wave_heights() {
        ...
        for glyph in s.chars() {
            if super::PROP_GLYPHS.contains(&glyph) && !distinct.contains(&glyph) {
                distinct.push(glyph);
            }
        }
        assert!(
            distinct.len() >= 5,
            "expected several propeller brightness levels, saw {distinct:?}",
        );
    }
