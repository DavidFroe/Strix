...
fn render_sidebar_auto(f: &mut Frame, area: Rect, app: &App) {
    #[derive(Clone, Copy)]
    enum Panel {
        Info,    // Vibecoding-Info: CWD, Projektname, Datei-Status mit Command-Hints
        Plan,
        Todos,
        Tasks,
        Agents,
    ...
    // User-Wunsch: Panels sind immer da, nicht erst bei Daten — sonst übersieht
    // man, dass sie existieren. Empty-Checks fließen nur in den Inhalt ("keine
    // aktiven Tasks") statt das Panel ganz auszublenden.
    let _ = (todos_empty, tasks_empty, agents_empty);
    let mut visible: Vec<Panel> = Vec::with_capacity(6);
    visible.push(Panel::Info);   // Workspace + Projektname + File-Status
    visible.push(Panel::Plan);   // Plan vom update_plan-Tool
    visible.push(Panel::Tasks);  // Aktive Background-Tasks
    visible.push(Panel::Todos);  // Todo-Liste
    visible.push(Panel::Agents); // Sub-Agents
    if app.context_panel {
        visible.push(Panel::Context); // opt-in via /context-toggle
    }

    // Info-Panel kriegt feste Länge (≈ 7 Zeilen reichen für Workspace,
    // Projektname, 3 Datei-Status + Hints + Spacer). Plan ist die zweitwichtigste
    // Sektion und kriegt Min(0); die übrigen teilen sich den Rest gleichmäßig.
    let constraints: Vec<Constraint> = {
        let mut c: Vec<Constraint> = Vec::with_capacity(visible.len());
        for (idx, panel) in visible.iter().enumerate() {
            match panel {
                Panel::Info => c.push(Constraint::Length(9)),
                _ if idx == visible.len() - 1 => c.push(Constraint::Min(6)),
                _ => c.push(Constraint::Percentage(
                    (90 / visible.len().saturating_sub(1).max(1)) as u16,
                )),
            }
        }
        c
    ...
    for (panel, rect) in visible.iter().zip(sections.iter()) {
        match panel {
            Panel::Info => render_sidebar_info(f, *rect, app),
    ...
    if app.task_panel.is_empty() {
        ...
        lines.push(Line::from(vec![
            Span::styled(
                ...
                Style::default().fg(palette::accent_light()).bold(),
            ),
            Span::styled(
                if running == app.task_panel.len() {
                    String::new()
...
pub fn subagent_navigator_lines(
    ...
    if summary.foreground_rlm_running {
        lines.push(Line::from(vec![
            Span::styled("RLM", Style::default().fg(palette::accent_light()).bold()),
            Span::styled(
...
// ── Info-Panel (Vibecoding-Anker) ────────────────────────────────────────
//
// Kompakte Übersicht: wo läuft propeller, wie heißt das Projekt, welche
// Vibecoding-Files existieren + welcher Slash-Command zeigt sie. Damit der
// User sich nie fragt "Mom, gibt's hier ne spec.md?". 9 Zeilen Constraint,
// reicht für Header + 3 Datenzeilen + 3 File-Status + Spacer.

fn render_sidebar_info(f: &mut Frame, area: Rect, app: &App) {
    if area.height < 3 {
        return;
    }
    let content_width = area.width.saturating_sub(4) as usize;
    ...
    // Workspace mit Home-Substitution kürzen wie im Header.
    let ws_full = ws.to_string_lossy().into_owned();
    let home = dirs::home_dir()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_default();
    let ws_short = if !home.is_empty() && ws_full.starts_with(&home) {
        format!("~{}", &ws_full[home.len()..])
    } else {
        ws_full.clone()
    };
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(
            truncate_line_to_width(&ws_short, content_width.saturating_sub(2).max(1)),
    ...
    lines.push(Line::from(vec![
        Span::styled("◉ ", Style::default().fg(palette::TEXT_MUTED)),
        Span::styled(
            truncate_line_to_width(&project, content_width.saturating_sub(2).max(1)),
            Style::default().fg(palette::accent_light()),
        ),
    ]));
...
/// Versucht aus dem Workspace einen Projektnamen abzuleiten. Reihenfolge:
/// 1. Erste H1-Headline (`# foo`) in CLAUDE.md, spec.md oder README.md
