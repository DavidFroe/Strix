...
pub enum ViewEvent {
    ...
    /// Emitted by the `/submodel` picker on Enter — `None` means clear the
    /// sub-agent override and inherit the main model again.
    SubmodelPickerApplied {
        model: Option<String>,
            ...
            .block(
                Block::default()
                    ...
                    .title_bottom(Line::from(vec![
                        ...
                        Span::styled(scroll_indicator, Style::default().fg(palette::accent_light())),
                    ]))
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(palette::accent_border()))
                    .style(Style::default().bg(palette::DEEPSEEK_INK))
    ...
    for agent in agents {
        ...
        if let Some(role) = agent.assignment.role.as_deref() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(role, Style::default().fg(palette::accent_light())),
            ]));
        }
...
fn format_agent_status(
    ...
    match status {
        SubAgentStatus::Running => ("running", Style::default().fg(palette::accent_light()), None),
