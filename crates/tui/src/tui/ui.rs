...
pub async fn run_tui(config: &Config, options: TuiOptions) -> Result<()> {
    ...
    // Push the persisted sub-agent model override (if any) so freshly spawned
    // sub-agents use it from the first turn onwards.
    let _ = engine_handle
        .send(Op::SetSubagentModel {
            model: app.subagent_model.clone(),
        })
        .await;
...
async fn run_event_loop(
    ...
    loop {
        ...
        // Poll startup model-inference (runs during splash screen).
        app.poll_startup_inference();
        ...
        // just dismiss the splash and continue.
        if app.splash.as_ref().map_or(false, |s| s.is_done() && s.inference_failed()) {
            app.splash = None;
        }
        ...
        // ── Bildschirmschoner ─────────────────────────────────────────────
        {
            let timeout = crate::tui::app::read_strix_conf_screensaver_timeout();
            let idle = now.duration_since(app.last_input_at);
            let splash_active = app.splash.is_some();
            let is_loading = app.is_loading;
            if app.screensaver.is_none() && !splash_active && !is_loading && idle >= timeout {
                let pool = crate::tui::app::build_screensaver_quote_pool();
                app.screensaver = Some(crate::tui::screensaver::Screensaver::new(pool));
            }
            if let Some(ss) = app.screensaver.as_mut() {
                let area = terminal.size()?;
                ss.tick(area.width, area.height);
                terminal.draw(|f| ss.render(f))?;
            }
        }
        // Screensaver aktiv: normales Draw unterdrücken; Tasteneingabe-Erkennung
        // übernimmt der Haupt-Event-Poll weiter unten (Lines 1660+).
        if app.screensaver.is_some() {
        ...
        // Screensaver- und Splash-Animation brauchen kurze Takt-Rate.
        if app.screensaver.is_some() || app.splash.as_ref().map_or(false, |s| !s.is_done()) {
        ...
        if event::poll(poll_timeout)? {
            ...
            // Jede Nutzereingabe → Idle-Timer zurücksetzen + Screensaver beenden.
            ...
            if is_user_input {
                ...
                if app.screensaver.take().is_some() {
                    // Vollständig neu zeichnen nach Screensaver-Ende.
                    reset_terminal_viewport(terminal)?;
                    app.needs_redraw = true;
                    continue;
                }
            ...
            // Splash keypress: dismiss early if the splash is still showing.
            if app.splash.as_ref().map_or(false, |s| !s.is_done()) {
                app.dismiss_splash();
                continue;
            }
            ...
            match key.code {
                ...
                // Ctrl+Tab: cycle through models the adapter reported as
                // available (status=OK or triage_available). Falls through
                // to plain Tab handling when the catalogue is empty or has
                // fewer than two entries, so users on terminals that map
                // Ctrl+Tab to plain Tab don't lose mode-cycling.
                KeyCode::Tab if key.modifiers.contains(KeyModifiers::CONTROL) => {
                    if let Some(new_model) = app.cycle_to_next_available_model() {
                        let display = crate::models::display_model_name(&new_model);
                        app.status_message = Some(format!("→ {display}"));
                        let _ = engine_handle
                            .send(Op::SetModel {
                                model: new_model,
                            })
                ...
                // Ctrl+N: terminal-universal fallback for model cycling.
                // Ctrl+Tab requires kitty/WezTerm keyboard protocol; most
                // terminals (GNOME Terminal, xterm) send plain Tab instead.
                // Ctrl+N always arrives as CONTROL+'n' and is otherwise unused.
                KeyCode::Char('n')
                    if key.modifiers.contains(KeyModifiers::CONTROL)
                        && !key.modifiers.contains(KeyModifiers::ALT)
                        && app.view_stack.is_empty() =>
                ...
                // Alt+Tab / Alt+M: cycle the sub-agent / background model
                // through the same catalogue. Alt+Tab is preferred but on
                // many desktop window-managers it's stolen for window
                // switching, so Alt+M is wired as an always-available
                // fallback (no other handler claims it). Both branches mutate
                // `app.subagent_model`, persist it, and push the change to
                // the engine via `Op::SetSubagentModel`.
                KeyCode::Tab if key.modifiers.contains(KeyModifiers::ALT) => {
                    if let Some(new_model) = app.cycle_to_next_available_subagent_model() {
                        let display = crate::models::display_model_name(&new_model);
                        app.status_message = Some(format!("sub → {display}"));
                        persist_subagent_model_setting(app.subagent_model.clone());
                        let _ = engine_handle
                            .send(Op::SetSubagentModel {
                                model: app.subagent_model.clone(),
                            })
                ...
                KeyCode::Tab => {
                    ...
                    if let Some((name, model, subagent)) = app.cycle_preset() {
                        app.model = model.clone();
                        app.subagent_model = Some(subagent.clone());
                        app.push_status_toast(
                            format!("→ {name}  ({model} / {subagent})"),
                            crate::tui::app::StatusToastLevel::Info,
                            Some(2_000),
                        );
                        let _ = engine_handle.send(Op::SetModel { model }).await;
                        let _ = engine_handle.send(Op::SetSubagentModel { model: Some(subagent) }).await;
                    } else {
                        let prior_model = app.model.clone();
                        app.cycle_mode();
                        if app.model != prior_model {
                            let _ = engine_handle
                                .send(Op::SetModel { model: app.model.clone() })
                                .await;
                        }
                    }
                }
...
async fn dispatch_user_message(
    ...
    if let Err(err) = engine_handle
        .send(Op::SendMessage {
            ...
            allow_shell: app.allow_shell,
            trust_mode: app.trust_mode,
            auto_approve: matches!(app.mode, AppMode::Yolo | AppMode::Propeller),
...
/// Best-effort persistence of the sub-agent model setting. Used by the
/// keyboard cycler (Alt+Tab / Alt+M) where we don't want a settings-write
/// failure to swallow the in-memory change or block the redraw — failures
/// surface only via the status line update from the caller.
fn persist_subagent_model_setting(model: Option<String>) {
    if let Ok(mut settings) = crate::settings::Settings::load() {
        settings.subagent_model = model;
        let _ = settings.save();
    }
}

/// Apply a sub-agent model change: mutate `app.subagent_model`, persist via
/// `Settings.subagent_model`, push `Op::SetSubagentModel` to the engine, and
/// surface a one-line status describing what changed. `None` clears the
/// override so future spawns inherit the main model.
async fn apply_subagent_model_change(
    app: &mut App,
    engine_handle: &EngineHandle,
    model: Option<String>,
) {
    let previous = app.subagent_model.clone();
    if previous == model {
        let label = match &model {
            Some(m) => crate::models::display_model_name(m),
            None => "inherit (main)".to_string(),
        };
        app.status_message = Some(format!("Sub-agent model unchanged: {label}"));
        return;
    }
    app.subagent_model = model.clone();

    let mut persist_warning: Option<String> = None;
    match crate::settings::Settings::load() {
        Ok(mut settings) => {
            settings.subagent_model = model.clone();
            if let Err(err) = settings.save() {
                persist_warning = Some(format!("(not persisted: {err})"));
            }
        }
    ...
    let previous_label = previous
        .as_deref()
        .map(crate::models::display_model_name)
        .unwrap_or_else(|| "inherit".to_string());
    let new_label = model
        .as_deref()
        .map(crate::models::display_model_name)
        .unwrap_or_else(|| "inherit (main)".to_string());
    let mut summary = format!("Sub-agent model: {previous_label} → {new_label}");
    if let Some(warning) = persist_warning {
        summary.push(' ');
        summary.push_str(&warning);
    }
...
async fn apply_command_result(
    ...
    if let Some(action) = result.action {
        match action {
            ...
            AppAction::OpenSubmodelPicker => {
                if app.view_stack.top_kind() != Some(ModalKind::SubmodelPicker) {
                    app.view_stack
                        .push(crate::tui::model_picker::ModelPickerView::new_for_subagent(app));
                }
            }
            AppAction::ApplySubagentModel { model } => {
                apply_subagent_model_change(app, engine_handle, model).await;
            }
...
fn render(f: &mut Frame, app: &mut App) {
    ...
    // Startup splash overlay (3 s, interactive TUI only — not exec/headless)
    if let Some(splash) = &app.splash {
        if !splash.is_done() {
            splash.render(size, f.buffer_mut());
            return;
        }
    }
    if app.splash.as_ref().map_or(false, |s| s.is_done()) {
    ...
    // Render header
    {
        ...
        // Volle Workspace-Pfad-Anzeige im Header — der User soll auf einen
        // Blick sehen WO propeller gerade propellert. Home wird zu `~` gekürzt,
        // den Rest übernimmt das truncate-Logic des Header-Widgets.
        let workspace_full = app.workspace.to_string_lossy();
        let home_string = dirs::home_dir()
            .map(|p| p.to_string_lossy().into_owned())
            .unwrap_or_default();
        let workspace_display: String = if !home_string.is_empty()
            && workspace_full.starts_with(&home_string)
        {
            format!("~{}", &workspace_full[home_string.len()..])
        } else {
            workspace_full.into_owned()
        };
        let workspace_name = if workspace_display.is_empty() {
            "workspace"
        } else {
            workspace_display.as_str()
        };
        let model_label = app.model_display_label();
        let subagent_model_label = app
            .subagent_model
            .as_deref()
            .map(crate::models::display_model_name);
        ...
        .with_subagent_model(subagent_model_label.as_deref())
        .with_usage(
    ...
    // ── Tall-Sidebar-Layout ──────────────────────────────────────────
    // Die Sidebar (Info/Plan/Tasks/...) erstreckt sich über die volle
    // Höhe von Body+Preview+Composer (alles zwischen Header und Footer).
    // Composer und Preview werden in der Breite gekürzt damit links
    // Platz für die Sidebar bleibt. Header und Footer bleiben full-width.
    // file-tree-Pane (falls aktiv) sitzt links in der Body-Zeile, frisst
    // also nur aus chat_area, nicht aus der Sidebar.
    let body_rect = chunks[1];
    let body_width = body_rect.width;
    let preferred_sidebar = (u32::from(body_width)
        * u32::from(app.sidebar_width_percent.clamp(10, 50))
        / 100) as u16;
    let sidebar_width_raw = preferred_sidebar
        .max(24)
        .min(body_width.saturating_sub(40));
    let sidebar_width = if body_width >= SIDEBAR_VISIBLE_MIN_WIDTH && sidebar_width_raw >= 20 {
        sidebar_width_raw
    } else {
        0
    };
    let sidebar_area_tall = if sidebar_width > 0 {
        Some(Rect {
            x: body_rect.x + body_width.saturating_sub(sidebar_width),
            y: body_rect.y,
            width: sidebar_width,
            height: body_rect.height + chunks[2].height + chunks[3].height,
        })
    } else {
        None
    };
    let left_width = body_width.saturating_sub(sidebar_width);
    let chat_outer = Rect {
        x: body_rect.x,
        y: body_rect.y,
        width: left_width,
    ...
    // Render chat + optional file-tree pane (innerhalb der linken Spalte)
    {
        // Defensive backstop (#400): den Body-Hintergrund zuerst füllen.
        Block::default()
            .style(Style::default().bg(app.ui_theme.surface_bg))
            .render(body_rect, f.buffer_mut());

        let chat_area =
            if app.file_tree.is_some() && chat_outer.width >= SIDEBAR_VISIBLE_MIN_WIDTH {
        ...
        if let Some(sidebar_rect) = sidebar_area_tall {
            super::sidebar::render_sidebar(f, sidebar_rect, app);
        }
    }

    // Render pending-input preview (queued/steered messages, if any).
    // Nutzt preview_area (gekürzt um sidebar_width) statt chunks[2] damit
    // die Tall-Sidebar nicht überzeichnet wird.
    if preview_height > 0 {
        let buf = f.buffer_mut();
        pending_preview.render(preview_area, buf);
    }

    // Render composer — composer_area (gekürzt) ist oben definiert.
    let (cursor_pos, slash_hint_visible) = {
        let composer_widget = ComposerWidget::new(
            app,
            composer_max_height,
            &slash_menu_entries,
        ...
        let hint_visible = composer_widget.slash_hint_visible(composer_area);
        let buf = f.buffer_mut();
        composer_widget.render(composer_area, buf);
        (composer_widget.cursor_pos(composer_area), hint_visible)
    };
    // composer_widget dropped — safe to write to app.viewport now.
    app.viewport.slash_hint_area = if slash_hint_visible && composer_area.height >= 1 {
        Some(Rect {
            x: composer_area.x,
            y: composer_area.y + composer_area.height - 1,
...
fn draw_app_frame(terminal: &mut AppTerminal, app: &mut App) -> Result<()> {
    ...
    // Synchronized Output Mode (DECSET 2026): tell the terminal to buffer
    // the entire frame and render it atomically. Eliminates per-cell tearing
    // and the flicker that's especially visible over SSH where each crossterm
    // write hits the wire as its own packet. Supported by Kitty, WezTerm,
    // Ghostty, Konsole, Windows Terminal, gnome-terminal ≥3.50, foot, recent
    // alacritty, xterm. Older terminals silently ignore the escape.
    {
        let backend = terminal.backend_mut();
        let _ = backend.write_all(b"\x1b[?2026h");
        let _ = backend.flush();
    }
    let draw_err = terminal.draw(|f| render(f, app)).err();
    {
        let backend = terminal.backend_mut();
...
fn push_keyboard_enhancement_flags<W: Write>(writer: &mut W) {
    // REPORT_ALL_KEYS_AS_ESCAPE_CODES is needed so terminals that speak the
    // kitty keyboard protocol surface modifiers on otherwise-bare keys like
    // Tab / Enter — Ctrl+Tab cycles the active model and that needs the
    // CONTROL modifier to come through.
    let flags = KeyboardEnhancementFlags::DISAMBIGUATE_ESCAPE_CODES
        | KeyboardEnhancementFlags::REPORT_ALL_KEYS_AS_ESCAPE_CODES;
    if let Err(err) = execute!(writer, PushKeyboardEnhancementFlags(flags)) {
        tracing::debug!(
...
fn status_color(level: StatusToastLevel) -> ratatui::style::Color {
    match level {
        StatusToastLevel::Info => palette::accent_light(),
...
fn render_footer(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    if footer_working_strip_active(app) {
        let now_ms = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        // Show subagent/tool status in cyan when available; otherwise leave
        // the state label as-is (the propeller animation in the spacer is the
        // visual "working" indicator).
        if let Some(lbl) = active_subagent_status_label(app).or_else(|| active_tool_status_label(app)) {
            props.state_label = lbl;
            props.state_color = palette::accent_light();
        }
...
fn render_footer_from(
    ...
    let (base_label, base_color) = if has(S::Status) {
    ...
    // Enrich the working label with elapsed time + phase so the user can
    // see something is happening even when the model is slow (e.g. 108 s TTFT).
    let (state_label, state_color): (String, _) = if app.is_loading {
        let elapsed = app
            .turn_started_at
            .map(|t| t.elapsed().as_secs())
            .unwrap_or(0);
        let phase = if app.streaming_thinking_active_entry.is_some() {
            "thinking"
        } else if app.streaming_message_index.is_some() {
            "generating"
        } else {
            "waiting"
        };
        let label = if elapsed > 0 {
            format!("{phase} {elapsed}s")
        } else {
            phase.to_string()
        };
        (label, app.ui_theme.status_working)
    } else {
        (base_label.to_string(), base_color)
    };
...
fn footer_coherence_spans(app: &App) -> Vec<Span<'static>> {
    ...
    let (label, color) = match app.coherence_state {
        ...
        CoherenceState::VerifyingRecentWork => ("verifying", palette::accent_light()),
...
fn handle_mouse_event(app: &mut App, mouse: MouseEvent) -> Vec<ViewEvent> {
    ...
    match mouse.kind {
        ...
        MouseEventKind::Down(MouseButton::Left) => {
            ...
            if mouse_hits_rect(mouse, app.viewport.slash_hint_area) {
                app.insert_char('/');
                return Vec::new();
            }
