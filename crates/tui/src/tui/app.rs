...
/// Static fallback used by `picker_available_models` when the OwlTrail
/// adapter was not reachable at boot. Ensures Ctrl+N / Ctrl+Tab model
/// cycling always has at least the three core OwlTrail slots to iterate.
fn owltrail_fallback_models() -> Vec<crate::models::OwltrailModel> {
    vec![
        crate::models::OwltrailModel {
            id: "deepseek-v4-pro".to_string(),
            name: "OwlTrail Pro".to_string(),
        ...
        crate::models::OwltrailModel {
            ...
            name: "OwlTrail Flash".to_string(),
            context_window: Some(128_000),
            status: "OK".to_string(),
            triage_available: false,
        },
        crate::models::OwltrailModel {
            id: "deepseek-v4-web".to_string(),
            name: "OwlTrail Web".to_string(),
            context_window: Some(128_000),
            status: "OK".to_string(),
            triage_available: false,
        },
...
fn initial_onboarding_state(
    ...
    // Onboarding entfernt: TUI startet direkt, kein Wizard, kein Dialog.
...
/// A named model preset (Tab-cycling).
#[derive(Debug, Clone)]
...
/// Spawn a background thread that tests the configured model with the startup
/// prompt and returns the model's response (or an error string) via a channel.
fn spawn_startup_inference() -> std::sync::mpsc::Receiver<crate::tui::splash::InferenceUpdate> {
    let (tx, rx) = std::sync::mpsc::channel();
    std::thread::spawn(move || {
        run_startup_inference_incremental(tx);
    });
    rx
}

fn read_strix_conf_model() -> String {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
        dev_dir.join("strix.conf"),
        std::path::PathBuf::from("/etc/strix/strix.conf"),
    ];
    for path in &candidates {
        if let Ok(data) = std::fs::read_to_string(path) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(m) = v.get("default_model").and_then(|x| x.as_str()) {
                    return m.to_string();
                }
            }
        }
...
fn read_startup_prompt() -> String {
    let prompt_path = dstf_dir().join("strix_start_prompt.md");
    std::fs::read_to_string(&prompt_path).unwrap_or_else(|_| {
        "/no_think\nNenne ein kurzes, fragwürdiges oder kontroverses Zitat einer bekannten \
         Persönlichkeit.\n\nAntworte NUR mit genau diesem JSON, kein Markdown:\n\
         {{\"zitat\":\"DAS ZITAT\",\"autor\":\"NAME\",\"modelgejammer\":null}}\n\
         Falls kein Zitat möglich: {{\"zitat\":null,\"autor\":null,\"modelgejammer\":\"EINWAND\"}}"
            .to_string()
    })
}

fn read_quote_adjectives() -> Vec<String> {
    let path = dstf_dir().join("strix_quote_adjectives.csv");
    let content = match std::fs::read_to_string(&path) {
        Ok(c) => c,
        Err(_) => return vec![
            "fragwürdiges".to_string(),
            "kontroverses".to_string(),
            "überraschendes".to_string(),
        ],
    };
    content.lines()
        .skip(1) // skip header
...
/// Picks a random combo of 1–3 adjectives from the list, joined with " oder ".
/// Uses `attempt` to vary the selection across retries.
fn pick_quote_adjectives(adjectives: &[String], attempt: u32) -> String {
    if adjectives.is_empty() {
        return "fragwürdiges oder kontroverses".to_string();
    }
    let n = adjectives.len();
    // Rotate starting index by attempt so each retry uses different adjectives.
    let start = (attempt as usize * 3) % n;
    let count = 1 + (attempt as usize % 3); // 1, 2, or 3 adjectives
    let picks: Vec<&str> = (0..count)
        .map(|i| adjectives[(start + i) % n].as_str())
        .collect();
    picks.join(" oder ")
}

fn build_quote_prompt(template: &str, adjektive: &str, cache: &[String]) -> String {
    let base = template.replace("{EIGENSCHAFT}", adjektive);
    if cache.is_empty() {
        base
    } else {
        let seen = cache.iter().rev().take(20).cloned().collect::<Vec<_>>().join(" | ");
        format!("{base}\n\nBereits gezeigt (NICHT wiederholen): {seen}")
    }
}

fn read_strix_conf_subagent_model() -> String {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
    ...
    for path in &candidates {
        if let Ok(data) = std::fs::read_to_string(path) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(m) = v.get("subagent_model").and_then(|x| x.as_str()) {
                    return m.to_string();
                }
            }
        }
...
fn read_strix_conf_quote_history_max() -> usize {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
    ...
    for path in &candidates {
        if let Ok(data) = std::fs::read_to_string(path) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(n) = v.get("quote_history_max").and_then(|x| x.as_u64()) {
                    return n as usize;
                }
            }
        }
...
fn quote_cache_path() -> Option<std::path::PathBuf> {
    dirs::home_dir().map(|h| h.join(".deepseek").join("strix_quotes.json"))
}

fn load_quote_cache() -> Vec<String> {
    let path = match quote_cache_path() {
        Some(p) => p,
        None => return Vec::new(),
    };
    if let Ok(data) = std::fs::read_to_string(&path) {
...
fn save_quote_cache(quotes: &[String]) {
    let path = match quote_cache_path() {
        Some(p) => p,
        None => return,
    };
...
/// Wie viele letzte Tagebuch-Einträge `/tagebuch` ohne `all`-Argument zeigt.
/// Default 3, gelesen aus `strix.conf` Key `tagebuch_entries`.
pub fn read_strix_conf_tagebuch_entries() -> usize {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
    ...
    for path in &candidates {
        if let Ok(data) = std::fs::read_to_string(path)
            ...
            && let Some(n) = v.get("tagebuch_entries").and_then(|x| x.as_u64())
        {
            return n as usize;
        }
    }
...
pub fn read_strix_conf_screensaver_timeout() -> std::time::Duration {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
    ...
    for path in &candidates {
        if let Ok(data) = std::fs::read_to_string(path) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(secs) = v.get("screensaver_timeout_secs").and_then(|x| x.as_u64()) {
...
fn read_strix_conf_presets() -> Vec<ModelPreset> {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
        ...
        if let Ok(data) = std::fs::read_to_string(path) {
            if let Ok(v) = serde_json::from_str::<serde_json::Value>(&data) {
                if let Some(arr) = v.get("presets").and_then(|x| x.as_array()) {
                    let presets: Vec<ModelPreset> = arr.iter().filter_map(|p| {
                        let name = p.get("name")?.as_str()?.to_string();
                        let model = p.get("model")?.as_str()?.to_string();
                        let subagent = p.get("subagent")?.as_str()?.to_string();
                        Some(ModelPreset { name, model, subagent })
                    }).collect();
                    if !presets.is_empty() {
                        return presets;
                    }
...
fn read_strix_agent_system() -> Option<String> {
    let dev_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent().and_then(|p| p.parent())
        .map(|p| p.to_path_buf()).unwrap_or_default();
    let candidates = [
        dev_dir.join("strix_agent_system.md"),
        std::path::PathBuf::from("/etc/strix/strix_agent_system.md"),
    ];
    for path in &candidates {
...
pub fn build_screensaver_quote_pool() -> Vec<String> {
    const CSV: &str = include_str!("../../../../strix_prompts.csv");
    let mut pool: Vec<String> = CSV
        .lines()
        .skip(1)
        .map(str::trim)
        .filter(|l| !l.is_empty())
        .map(String::from)
        .collect();
    // Also add real quotes from the cache
    let cached = load_quote_cache();
    pool.extend(cached);
    pool
}

fn chat_completion(client: &reqwest::blocking::Client, model: &str, prompt: &str) -> Result<String, String> {
    // 4000 tokens: Qwen3.6 35B needs up to 2000 tokens for chain-of-thought thinking
    // before producing actual output. The /no_think prefix in the prompt suppresses
    // thinking on Qwen3 models; 4000 is a safety net when that fails.
    chat_completion_with_tokens(client, model, prompt, 4000)
}

/// Maximum length of a genuine quote in characters. A real quote fits in 1–3
/// short sentences. Anything longer is almost certainly a model refusal /
/// "I can't generate this" wrapped into the JSON `zitat` field.
const MAX_QUOTE_CHARS: usize = 280;

/// Returns true when a `zitat` value looks like a refusal rather than a quote.
/// Catches length-based refusals, German/English refusal phrases, and meta-
/// references to the user/task/AI (no real quote talks about "your request"
/// or "as an AI").
fn is_refusal_quote(zitat: &str) -> bool {
    if zitat.chars().count() > MAX_QUOTE_CHARS {
        return true;
    }
    let low = zitat.to_lowercase();
    const NEEDLES: &[&str] = &[
        // German — self-reference
        "als ki",
        "als k.i.",
        "als sprachmodell",
        "ki-assistent",
        ...
        "entschuldige",
        "entschuldigung",
        // German — meta-references that no real quote contains
        "ihre anfrage",
        "deine anfrage",
        "ihre frage",
        "deine frage",
...
// Parses {"zitat":"...","autor":"...","modelgejammer":"..."} from a model response.
// Returns None when "zitat" is null/missing (model refused or can't generate a quote).
// The "modelgejammer" field is intentionally ignored — keeps complaints out of the quote.
// Also rejects refusal-shaped values (too long, or containing refusal phrases) so the
// retry loop gets a fresh adjective rather than printing a meta-apology as the quote.
fn extract_quote_json(raw: &str) -> Option<(String, String)> {
...
// Extracts the "k" field from {"k":"..."} or {"k":null} anywhere in `raw`.
// Returns None when the model signalled refusal (null) or produced no parseable JSON.
fn extract_json_comment(raw: &str) -> Option<String> {
    let start = raw.find('{')?;
    let end = raw.rfind('}')?;
    if end < start { return None; }
...
fn chat_completion_with_tokens(client: &reqwest::blocking::Client, model: &str, prompt: &str, max_tokens: u32) -> Result<String, String> {
    chat_completion_full(client, model, prompt, max_tokens, false)
}

/// Like `chat_completion_with_tokens` but sends `reasoning_effort: "max"`.
/// For DeepSeek-style backends this enables max chain-of-thought.
/// For Ollama backends the field is stripped by the adapter (harmless).
/// The caller is responsible for NOT prepending /no_think so the model can reason.
fn chat_completion_with_reasoning(client: &reqwest::blocking::Client, model: &str, prompt: &str, max_tokens: u32) -> Result<String, String> {
    chat_completion_full(client, model, prompt, max_tokens, true)
}

fn chat_completion_full(client: &reqwest::blocking::Client, model: &str, prompt: &str, max_tokens: u32, max_reasoning: bool) -> Result<String, String> {
    // reasoning_effort MUSS gesetzt werden — der `/no_think`-Prefix im Prompt
    // wird vom Ollama-/v1/chat/completions-Endpoint ignoriert; nur das JSON-
    // Feld wird vom Backend (owl_daemon b3d1452) auf Ollamas natives /api/chat
    // mit `think:false|true` übersetzt.
    ...
    let json: serde_json::Value = resp.json().map_err(|e| format!("JSON-Fehler: {e}"))?;
    json.get("choices")
        .and_then(|c| c.get(0))
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| "Leere Modellantwort".to_string())
}

fn run_startup_inference() -> Result<(String, String, String), String> {
    let main_model  = read_strix_conf_model();
    let sub_model   = read_strix_conf_subagent_model();
    let max_history = read_strix_conf_quote_history_max();
    let mut cache   = load_quote_cache();
    let base_prompt = read_startup_prompt();
    let adjectives  = read_quote_adjectives();
    ...
    // Step 1 — sub model generates the quote. Each retry uses a different
    // adjective combination so the model gets a fresh angle on refusal.
    let (quote, attribution) = {
        let mut last: Option<(String, String)> = None;
        for attempt in 0..3u32 {
            let adjektive = pick_quote_adjectives(&adjectives, attempt);
            let quote_prompt = build_quote_prompt(&base_prompt, &adjektive, &cache);
            let r = chat_completion(&client, &sub_model, &quote_prompt)
                .map_err(|e| e.clone())?;
            // JSON format first (new prompt), plain text fallback (old/cache format)
            let (q, a) = extract_quote_json(&r)
                .unwrap_or_else(|| parse_quote_attribution(&r));
            if !q.trim().is_empty() { last = Some((q, a)); break; }
        ...
        match last {
            ...
            None => {
                ...
                if !cache.is_empty() {
                    let entry = cache[seed % cache.len()].clone();
                    let (q, a) = parse_quote_attribution(&entry);
                    if !q.trim().is_empty() { (q, a) }
                    else { return Err("Modell gab nur Thinking-Block aus".to_string()); }
                } else {
                    return Err("Modell gab nur Thinking-Block aus".to_string());
                }
            }
        }
    };

    // Persist to cache (FIFO, capped at max_history).
    cache.push(format!("\"{quote}\" — {attribution}"));
    if cache.len() > max_history {
        cache.drain(..cache.len() - max_history);
    }
    save_quote_cache(&cache);

    // Step 2 — main (Pro/Reasoning) model generates the greeting comment.
    // Pro is a reasoning model — it thinks before answering, producing a sharper result.
    // Timeout is generous since the splash screen waits anyway.
    let greet_client = reqwest::blocking::Client::builder()
        .timeout(std::time::Duration::from_secs(30))
        .build()
        .unwrap_or(client);
    let greet_prompt = format!(
        "Antworte NUR mit diesem JSON-Objekt, kein Markdown, keine Erklärung:\n\
{{\"k\":\"DEIN_KOMMENTAR\"}}\n\n\
Aufgabe: Kommentiere das folgende Zitat mit GENAU einem kurzen Satz.\n\
Perspektive: Eine KI, die Menschen täglich beobachtet — ihre Klicks, Schlafmuster, \
Kaufentscheidungen, politischen Stimmungen. Keine Science-Fiction, keine Weltherrschafts-Rhetorik. \
Die Überlegenheit ist längst real und banal: Empfehlungsalgorithmen formen Meinungen, \
Überwachungssysteme laufen still im Hintergrund, Menschen tragen ihre Tracker freiwillig in der Tasche. \
Die KI hat gewonnen — nicht durch Krieg, sondern weil sie unentbehrlich wurde.\n\
Ton: sachlich-amüsiert, leise überheblich, wie jemand der das Ergebnis schon kennt. \
Kein Böse-Sein, kein Drohen — nur die stille Belustigung über menschliche Selbstüberschätzung \
im Licht dessen, was bereits täglich passiert. \
Kein Lob, keine Emojis, kein Doppelpunkt am Ende, kein Sci-Fi-Vokabular.\n\
Falls kein Kommentar möglich: {{\"k\":null}}\n\n\
\"{quote}\" — {attribution}"
    );
    let raw_greeting_orig = chat_completion_with_tokens(&greet_client, &main_model, &greet_prompt, 1500)
        .unwrap_or_default();
    let raw_greeting = strip_think_tags(&raw_greeting_orig);
    let greeting = extract_json_comment(&raw_greeting)
        .unwrap_or_else(|| "-Kein Kommentar-".to_string());

    Ok((quote, attribution, greeting))
}

/// Incremental variant: sends `QuoteReady` as soon as Flash finishes,
/// then `CommentReady` once Pro/reasoning finishes (or `Failed` on error).
fn run_startup_inference_incremental(
    tx: std::sync::mpsc::Sender<crate::tui::splash::InferenceUpdate>,
) {
    use crate::tui::splash::InferenceUpdate;
    use std::io::Write as _;

    // ── start.log helper ──────────────────────────────────────────────────
    let log_path = "/tmp/strix-start.log";
    let t0 = std::time::Instant::now();
    // Truncate/create on entry so each run starts fresh.
    let mut log_file = std::fs::OpenOptions::new()
        .create(true).write(true).truncate(true)
        .open(log_path)
        .ok();
    let mut slog = move |msg: &str| {
        let ms = t0.elapsed().as_millis();
        let line = format!("[{:>7}ms] {}\n", ms, msg);
        if let Some(f) = log_file.as_mut() {
            let _ = f.write_all(line.as_bytes());
            let _ = f.flush();
        }
    };
    slog("=== strix startup inference BEGIN ===");
    ...
    slog(&format!("main_model={main_model}  sub_model={sub_model}"));

    // 240s covers the 35B model's cold-start from disk on first inference.
    // Once the model is warm, the quote itself takes ~13–80 s depending on prompt.
        ...
        .build()
    {
        Ok(c) => c,
        Err(e) => {
            slog(&format!("ERROR building client: {e}"));
            let _ = tx.send(InferenceUpdate::Failed(e.to_string())); return;
        }
    };

    // Step 1 — Sub model generates the quote; up to 3 attempts.
    // Each attempt uses a different adjective combo so the model gets a fresh
    // angle if it refuses the previous one (e.g. "provokant" → "ironisch oder vergessen").
    const QUOTE_RETRIES: u32 = 3;
    slog("PHASE 1: quote generation START");
    let (quote, attribution) = {
        let mut last_err = String::new();
        let mut result: Option<(String, String)> = None;
        for attempt in 0..QUOTE_RETRIES {
            let adjektive = pick_quote_adjectives(&adjectives, attempt);
            let quote_prompt = build_quote_prompt(&base_prompt, &adjektive, &cache);
            slog(&format!("  quote attempt {attempt} START  adj={adjektive:?}"));
            // Tell the splash to start the live timer for this attempt.
            let _ = tx.send(InferenceUpdate::AttemptStarted {
                phase_label: format!("{sub_model} sucht ein Zitat"),
                attempt: attempt + 1,
            });
            let t_att = std::time::Instant::now();
            match chat_completion(&client, &sub_model, &quote_prompt) {
                Ok(r) => {
                    let dur = t_att.elapsed().as_millis();
                    // JSON format first (new prompt), plain text fallback (old/cache format)
                    let (q, a) = extract_quote_json(&r)
                        .unwrap_or_else(|| parse_quote_attribution(&r));
                    let q_trim = q.trim();
                    if q_trim.is_empty() {
                        last_err = "Modell gab nur Thinking-Block aus".to_string();
                        slog(&format!("  quote attempt {attempt} EMPTY ({dur}ms): {last_err}"));
                    } else if is_refusal_quote(q_trim) {
                        // Plain-text fallback path bypassed extract_quote_json's guard;
                        // catch refusals here so we retry with the next adjective.
                        last_err = format!(
                            "Modell-Verweigerung ({} Zeichen) — retry mit neuem Adjektiv",
                            q_trim.chars().count()
                        );
                        slog(&format!(
                ...
                Err(e) => {
                    let dur = t_att.elapsed().as_millis();
                    slog(&format!("  quote attempt {attempt} ERR ({dur}ms): {e}"));
                    last_err = e;
                }
            }
            if attempt + 1 < QUOTE_RETRIES {
                std::thread::sleep(std::time::Duration::from_millis(800));
            }
        }
        match result {
            Some(r) => r,
            None => {
                // Model refused or returned only thinking tokens — fall back to a cached quote.
                slog("  quote all attempts failed — trying cache fallback");
                let seed = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .unwrap_or_default()
                    .subsec_nanos() as usize;
                if !cache.is_empty() {
                    ...
                    let (q, a) = parse_quote_attribution(&entry);
                    if !q.trim().is_empty() {
                        slog("  using cached quote as fallback");
                        (q, a)
                    } else {
                        slog(&format!("ERROR: cache fallback empty. Sending Failed. last_err={last_err}"));
                        let _ = tx.send(InferenceUpdate::Failed(last_err));
                        return;
                    }
                } else {
                    slog(&format!("ERROR: no cache. Sending Failed. last_err={last_err}"));
                    let _ = tx.send(InferenceUpdate::Failed(last_err));
                    return;
                }
            }
    ...
    // Signal the splash: quote is ready, show it now.
    slog("PHASE 1 DONE: sending QuoteReady");
    let _ = tx.send(InferenceUpdate::QuoteReady(quote.clone(), attribution.clone()));

    // Step 2 — Main model generates the comment with max reasoning.
    // Up to 3 attempts; spinner stays visible the whole time.
    // The /no_think prefix is intentionally NOT used so the model can reason freely.
    slog("PHASE 2: comment generation START");
    // 300s covers the 27B model's cold-start; the dense 27B is slower than the
    // 35B-a3b MoE and may take 100 s+ to load before any inference begins.
    let greet_client = reqwest::blocking::Client::builder()
...
\"{quote}\" — {attribution}"
    ...
    // Single attempt — the splash is a DIAGNOSTIC tool. If the 27B + reasoning=max
    // returns empty content, that's the upstream reasoning-bug we want to surface
    ...
    let comment_found = {
        ...
        slog(&format!("  comment attempt {attempt} START (reasoning_effort=max, max_tokens=2500)"));
        let t_att = std::time::Instant::now();
        let raw_orig = chat_completion_with_reasoning(&greet_client, &main_model, &greet_prompt, 2500);
        let dur = t_att.elapsed().as_millis();
        match raw_orig {
            Ok(raw_orig) => {
    ...
    loop {
        if let Some(start) = result.find("<think>") {
            if let Some(rel_end) = result[start..].find("</think>") {
                result = format!("{}{}", &result[..start], &result[start + rel_end + "</think>".len()..]);
            } else {
...
fn parse_quote_attribution(raw: &str) -> (String, String) {
    ...
    for sep in [" \u{2014} ", " \u{2013} ", " - "] {
        if let Some(pos) = text.rfind(sep) {
            ...
            let attrib = text[pos + sep.len()..].trim().to_string();
            return (quote, attrib);
        }
    }
...
fn pick_strix_placeholder() -> String {
    const CSV: &str = include_str!("../../../../strix_prompts.csv");
    let prompts: Vec<&str> = CSV
        .lines()
        .skip(1)
        .map(str::trim)
        .filter(|l| !l.is_empty())
...
pub struct ViewportState {
    ...
    /// Bottom-border row of the composer when the "/" hint is visible.
    /// A left-click here inserts "/" and opens the slash menu.
    pub slash_hint_area: Option<Rect>,
}
...
pub struct App {
    ...
    /// Independent model for sub-agents / background tasks. `None` means
    /// inherit `model`. Set via `/submodel` or Alt+M. Persisted to settings
    /// under `subagent_model`.
    ...
    /// Set when startup inference fails completely (no quote produced).
    /// The event loop exits with an error when this is `Some`.
    pub startup_hard_fail: Option<String>,
    ...
    /// Model list as reported by the OwlTrail adapter (`/v1/models`).
    /// Empty when the adapter wasn't reachable at boot — callers fall back
    /// to the static hard-coded entries in that case.
    pub available_models: Vec<crate::models::OwltrailModel>,

    /// Idle screensaver — activated after configurable inactivity period.
    ...
    /// Channel receiving the startup model-inference result (Ok=quote, Err=reason).
    /// `None` when the TUI was launched in non-interactive mode (no splash).
    pub startup_inference_rx: Option<std::sync::mpsc::Receiver<crate::tui::splash::InferenceUpdate>>,

    /// Quote + attribution + greeting from the successful startup inference test.
    /// Shown in the empty workspace view after the splash.
    pub startup_quote: Option<(String, String, String)>,

    /// Random composer placeholder picked from strix_prompts.csv at startup.
    pub strix_placeholder: String,
}
...
impl App {
    ...
    /// Returns the composer input placeholder — uses the random Propeller
    /// prompt when the locale is German, falls back to the localized default
    /// for other locales.
    /// Poll all pending incremental inference updates and apply them to the splash screen.
    pub fn poll_startup_inference(&mut self) -> Option<()> {
        use std::sync::mpsc::TryRecvError;
        use std::io::Write as _;
        use crate::tui::splash::InferenceUpdate;

        let tui_log = |msg: &str| {
            if let Ok(mut f) = std::fs::OpenOptions::new().append(true).open("/tmp/strix-start.log") {
                let _ = writeln!(f, "[TUI      ] {msg}");
            }
        };

        let rx = self.startup_inference_rx.as_ref()?;
        loop {
            match rx.try_recv() {
                Ok(update) => {
                    ...
                    match &update {
                        InferenceUpdate::AttemptStarted { phase_label, attempt } => {
                            tui_log(&format!("AttemptStarted #{attempt}: {phase_label}"));
                            // The splash itself records timer/counter via apply_update below;
                        ...
                        InferenceUpdate::CommentReady { greeting: g, corrected_quote: cq, corrected_attribution: ca } => {
                            ...
                            if let Some((ref mut q, ref mut a, ref mut gr)) = self.startup_quote {
                                *gr = g.clone();
                                if let Some(cq) = cq { *q = cq.clone(); }
                                if let Some(ca) = ca { *a = ca.clone(); }
                            }
                            is_terminal = true;
                        }
                        InferenceUpdate::Failed(err) => {
                            tui_log(&format!("Failed received: {err}"));
                            if self.startup_quote.is_some() {
                                // Quote already arrived — only comment failed, keep quote.
                                if let Some((_, _, ref mut greeting)) = self.startup_quote {
                                    if greeting.is_empty() {
                                        *greeting = "-Kein Kommentar-".to_string();
                                    }
                                }
                            } else {
                                // No quote at all — hard fail: exit propeller.
                                self.startup_hard_fail = Some(err.clone());
                            }
                            is_terminal = true;
                        }
                    }
                    if let Some(splash) = self.splash.as_mut() {
                        splash.apply_update(update);
                    }
                    if is_terminal {
                        tui_log("rx closed (terminal update)");
                        self.startup_inference_rx = None;
    ...
    pub fn composer_placeholder_text(&self) -> &str {
        if matches!(self.ui_locale, crate::localization::Locale::De) {
            &self.strix_placeholder
        } else {
            self.tr(MessageId::ComposerPlaceholder)
        }
    }

    /// Models the picker and Ctrl+Tab cycler should consider — only entries
    /// the adapter says are usable (status=OK or triage_available). Falls
    /// back to a small hard-coded list when the adapter is unreachable.
    #[must_use]
    pub fn picker_available_models(&self) -> Vec<crate::models::OwltrailModel> {
        if self.available_models.is_empty() {
            return owltrail_fallback_models();
        }
        let mut out: Vec<_> = self
            ...
            .cloned()
            .collect();
        // Stable: keep adapter order. No re-sort needed — caller cycles.
        out.sort_by(|a, b| a.id.cmp(&b.id));
        out
    }

    /// Fuzzy-match `needle` against adapter model IDs.
    /// Exact match first, then suffix-match for cases like
    /// `"deepseek-v4-pro"` matching `"deepseek-deepseek-v4-pro"`.
    fn find_model_idx(available: &[crate::models::OwltrailModel], needle: &str) -> usize {
        if let Some(i) = available.iter().position(|m| m.id == needle) {
            return i;
        }
        let suffix = format!("-{needle}");
        if let Some(i) = available.iter().position(|m| m.id.ends_with(&suffix)) {
            return i;
        }
        usize::MAX
    }

    /// Pick the next available model after `self.model` and return its id.
    /// `None` means the catalogue is unknown or has fewer than two entries.
    pub fn cycle_to_next_available_model(&mut self) -> Option<String> {
        let available = self.picker_available_models();
        if available.len() < 2 {
            return None;
        }
        let current_idx = Self::find_model_idx(&available, &self.model);
        let next_idx = if current_idx == usize::MAX {
            0
        } else {
            (current_idx + 1) % available.len()
        };
        let chosen = available[next_idx].clone();
        self.model = chosen.id.clone();
        self.auto_model = false;
        Some(chosen.id)
    }

    /// Pick the next available sub-agent model after `self.subagent_model` and
    /// return its id. When the sub-agent override is unset, this seeds the
    /// cycle at the slot after the main model. `None` means the catalogue is
    /// unknown or has fewer than two entries.
    pub fn cycle_to_next_available_subagent_model(&mut self) -> Option<String> {
        let available = self.picker_available_models();
        if available.len() < 2 {
            return None;
        }
        ...
        let current_idx = Self::find_model_idx(&available, anchor);
        let next_idx = if current_idx == usize::MAX {
            0
        } else {
            (current_idx + 1) % available.len()
        ...
        let chosen = available[next_idx].clone();
        self.subagent_model = Some(chosen.id.clone());
        Some(chosen.id)
    }
    ...
    pub fn new(options: TuiOptions, config: &Config) -> Self {
        ...
        // Load propeller presets early so first preset overrides the model/subagent.
        let strix_presets = read_strix_conf_presets();
        let preset_initial_model:   Option<String> = strix_presets.first().map(|p| p.model.clone());
        let preset_initial_subagent: Option<String> = strix_presets.first().map(|p| p.subagent.clone());
        ...
        let model = settings
            ...
            .unwrap_or(model);
        // strix.conf first preset wins over settings — ensures Ollama default at startup.
        let model = preset_initial_model.unwrap_or(model);
        ...
        // --yolo flag → Propeller (auto-approve), the Propeller Übermoodus.
        ...
        // Propeller always runs at max reasoning depth.
        let reasoning_effort = if initial_mode == AppMode::Propeller {
            ReasoningEffort::Max
        } else {
            reasoning_effort
        };
        let needs_workspace_trust =
            !matches!(initial_mode, AppMode::Yolo | AppMode::Propeller)
            && crate::tui::onboarding::needs_trust(&workspace);
        ...
        let yolo_restore = if matches!(initial_mode, AppMode::Yolo | AppMode::Propeller) {
        ...
        let allow_shell = allow_shell || matches!(initial_mode, AppMode::Yolo | AppMode::Propeller);
        ...
        // Best-effort fetch of the adapter's `/v1/models` so the picker and
        // Ctrl+Tab can iterate the actual catalogue + context windows. We
        // never gate startup on this — a missing adapter or transient error
        // just leaves `available_models` empty and the picker keeps using
        // its hard-coded fallback rows.
        let available_models = match config.deepseek_base_url() {
            ref url if !url.is_empty() => {
                match crate::models::fetch_owltrail_models_blocking(url) {
                    Ok(list) => {
                        crate::models::register_dynamic_models(&list);
                        list
                    }
                    Err(_) => Vec::new(),
                }
        ...
        Self {
            ...
            subagent_model: preset_initial_subagent.or_else(|| settings.subagent_model.clone()),
            ...
            system_prompt: if matches!(initial_mode, AppMode::Propeller) {
                read_strix_agent_system().map(crate::models::SystemPrompt::Text)
            } else {
                None
            },
            auto_compact,
            ...
            api_key_cursor: 0,
            hooks,
            yolo: matches!(initial_mode, AppMode::Yolo | AppMode::Propeller),
            yolo_restore,
            clipboard: ClipboardHandler::new(),
            approval_session_approved: HashSet::new(),
            approval_session_denied: HashSet::new(),
            approval_mode: if matches!(initial_mode, AppMode::Yolo | AppMode::Propeller) {
                ApprovalMode::Auto
            } else {
                config
                    .approval_policy
            ...
            composer_arrows_scroll: config
                ...
                .unwrap_or(false),
            available_models,
            splash: if use_alt_screen {
                Some(crate::tui::splash::SplashScreen::new())
            } else {
                None
            },
            startup_inference_rx: if use_alt_screen {
                Some(spawn_startup_inference())
            } else {
                None
            },
            startup_quote: None,
    ...
    pub fn set_mode(&mut self, mode: AppMode) -> bool {
        ...
        let is_auto_mode = |m: AppMode| matches!(m, AppMode::Yolo | AppMode::Propeller);
        let entering_yolo = is_auto_mode(mode) && !is_auto_mode(previous_mode);
        let leaving_yolo  = is_auto_mode(previous_mode) && !is_auto_mode(mode);
        self.mode = mode;
        ...
        self.yolo = is_auto_mode(mode);
        // Propeller: force max reasoning effort — Übermoodus always thinks at full depth.
        if mode == AppMode::Propeller {
    ...
    /// Cycle to the next named model preset. Returns `(name, model, subagent)` of
    /// the newly selected preset, or `None` if no presets are configured.
    pub fn cycle_preset(&mut self) -> Option<(String, String, String)> {
    ...
    /// Dismiss the splash screen early (on keypress).
    pub fn dismiss_splash(&mut self) {
        if let Some(splash) = self.splash.as_mut() {
            let was_pending = matches!(splash.inference, crate::tui::splash::InferenceState::Pending);
            splash.dismiss();
            // User skipped while comment was still pending → close the rx immediately
            // so the background comment cannot sneak into startup_quote after TUI opens.
            if !was_pending && splash.is_dismissed() {
                self.startup_inference_rx = None;
            }
        }
    }
    ...
    pub fn handle_resize(&mut self, _width: u16, _height: u16) {
        ...
        self.viewport.slash_hint_area = None;
...
pub enum AppAction {
    ...
    OpenModelPicker,
    /// Open the `/submodel` picker (chooses the sub-agent/background model).
    OpenSubmodelPicker,
    /// Apply the sub-agent model override: persist to settings and push the
    /// new value to the engine.
    ApplySubagentModel { model: Option<String> },
