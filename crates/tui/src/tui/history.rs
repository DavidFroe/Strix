...
fn checklist_status_marker(status: &str) -> (&'static str, Color) {
    match status.to_ascii_lowercase().as_str() {
        ...
        "in_progress" | "inprogress" | "running" => ("\u{25D0}", palette::accent_light()), // ◐
...
/// - Lines starting with `@@` => `palette::accent_light()` (cyan/blue)
...
fn assistant_label_style_for(streaming: bool, low_motion: bool) -> Style {
    let color = if streaming && !low_motion {
        let now_ms = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        palette::pulse_brightness(palette::accent_light(), now_ms)
    } else {
...
mod tests {
    ...
    fn assistant_glyph_holds_full_brightness_when_idle() {
        ...
        assert_eq!(idle.fg, Some(palette::accent_light()));
        assert_eq!(low_motion.fg, Some(palette::accent_light()));
    }
    ...
    fn assistant_glyph_pulses_when_streaming_and_motion_allowed() {
        ...
        for _ in 0..50 {
            if let Some(Color::Rgb(_, _, b)) = assistant_label_style_for(true, false).fg {
                let Color::Rgb(_, _, src_b) = palette::accent_light() else {
