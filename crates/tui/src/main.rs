...
#[command(
    ...
    about = "Propeller — TUI/CLI for OwlTrail models",
    long_about = "Propeller: terminal-native TUI and CLI for OwlTrail models.\n\nRun 'propeller' to start.\n\nNot affiliated with DeepSeek Inc."
)]
struct Cli {
    /// Subcommand to run
    #[command(subcommand)]
    ...
    /// Model to start with (owltrail-pro, owltrail-flash, owltrail-web, …).
    /// Falls back to $DEEPSEEK_MODEL, then to the config default.
    #[arg(short = 'm', long = "model", value_name = "MODEL")]
    model: Option<String>,

    /// Reasoning effort tier: off, low, medium, high, max, auto.
    /// Falls back to $DEEPSEEK_REASONING_EFFORT, then to the config default.
    #[arg(
        short = 'e',
        long = "effort",
        value_name = "LEVEL",
    ...
    /// Override the context-window cap (tokens) for the selected model.
    /// Useful to cap memory use below the adapter-advertised maximum.
    #[arg(long = "context", value_name = "TOKENS", global = true)]
    context: Option<u32>,

    /// Enable thinking/reasoning (same as Shift+Tab in the TUI).
    /// Equivalent to --effort high when --effort is not specified.
    #[arg(long = "think", global = true, conflicts_with = "no_think")]
    think: bool,

    /// Disable thinking/reasoning. Equivalent to --effort off.
    #[arg(long = "no-think", global = true, conflicts_with = "think")]
    no_think: bool,
}
...
async fn main() -> Result<()> {
    ...
    // Read theme from strix.conf
    {
        let theme_name = std::fs::read_to_string("/etc/strix/strix.conf")
            .or_else(|_| {
                let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
                    .parent().and_then(|p| p.parent())
                    .map(|p| p.join("strix.conf"))
                    .unwrap_or_default();
        ...
        return match command {
            ...
            Commands::Exec(args) => {
                ...
                let model = args
                    .model
                    .map(|m| {
                        crate::config::canonical_model_name(m.trim())
                            .map(str::to_string)
                            .unwrap_or_else(|| m.trim().to_string())
                    })
                    .or_else(|| config.default_text_model.clone())
                    .unwrap_or_else(|| config.default_model());
                // Re-key the --context override to the actually-resolved
                // model for this exec run (the subcommand-level --model can
                // differ from cli.model, which is what apply_cli_runtime_overrides
                // saw at load time).
                if let Some(tokens) = cli.context
                    && tokens > 0
                {
                    crate::models::set_context_window_override(&model, tokens);
                }
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    println!("  propeller: {}", env!("DEEPSEEK_BUILD_VERSION"));
...
fn list_sessions(limit: usize, search: Option<String>) -> Result<()> {
    ...
    println!(
        "Resume with: {} {}",
        "propeller --resume".truecolor(blue_r, blue_g, blue_b),
        "<session-id>".dimmed()
    );
    println!(
        "Continue latest in this workspace: {}",
        "propeller --continue".truecolor(blue_r, blue_g, blue_b)
    );
...
fn init_project() -> Result<()> {
    ...
    match create_default_agents_md(&workspace) {
        Ok(path) => {
            ...
            println!("The instructions will be loaded automatically when you run propeller.");
        }
        Err(e) => {
            println!(
...
/// Apply runtime CLI overrides (--model, --effort, --context, --think,
/// --no-think) onto the loaded config. `--effort` and `--think`/`--no-think`
/// also honor the `DEEPSEEK_REASONING_EFFORT` env-var fallback. `--context`
/// writes through to the dynamic context-window registry under the resolved
/// model id so the rest of the TUI picks it up.
fn apply_cli_runtime_overrides(config: &mut Config, cli: &Cli) {
    if let Some(raw) = cli.model.as_deref() {
        let trimmed = raw.trim();
        if !trimmed.is_empty() {
            let resolved = crate::config::canonical_model_name(trimmed)
                .map(str::to_string)
                .unwrap_or_else(|| trimmed.to_string());
            config.default_text_model = Some(resolved);
        }
    }

    let effort_override = if let Some(raw) = cli.effort.as_deref() {
        let trimmed = raw.trim();
        (!trimmed.is_empty()).then(|| trimmed.to_ascii_lowercase())
    } else if cli.no_think {
        Some("off".to_string())
    } else if cli.think {
        Some("high".to_string())
    } else {
    ...
    if let Some(tokens) = cli.context {
        if tokens > 0 {
            let model_for_ctx = config
                .default_text_model
                .clone()
                .unwrap_or_else(|| config.default_model());
            crate::models::set_context_window_override(&model_for_ctx, tokens);
        }
    }
}
...
fn recover_interrupted_checkpoint_for_resume(launch_workspace: &Path) -> Option<String> {
    ...
    if !workspace_matches {
        ...
        let _ = manager.clear_checkpoint();
        logging::info(format!(
            "interrupted session from another workspace ({}) preserved; \
             starting fresh in {}.",
    ...
    logging::info(format!(
        "recovered interrupted session ({age_str}); use --fresh to start fresh."
    ));
...
fn preserve_interrupted_checkpoint_for_explicit_resume(launch_workspace: &Path) {
    ...
    if session_manager::workspace_scope_matches(&session_workspace, launch_workspace) {
        logging::info(format!(
            "in-flight session snapshot ({age_str}) preserved; starting new session."
        ));
    } else {
        logging::info(format!(
            "interrupted session from another workspace ({}) preserved; \
...
fn merge_project_config(config: &mut Config, workspace: &Path) {
    ...
    for key in DENY_AT_PROJECT_SCOPE {
        if table.contains_key(*key) {
            logging::warn(format!(
                "project-scope config key `{key}` is ignored — \
    ...
    for (key, field) in [
            ...
            && !v.is_empty()
        {
            ...
            if is_escalation {
                logging::warn(format!(
                    "project-scope `{key} = \"{v}\"` is ignored — \
...
async fn run_interactive(
    ...
    if !cli.no_project_config {
        ...
        // Re-apply CLI runtime overrides so an explicit --model / --effort /
        // --context still wins over a project's .deepseek/config.toml.
        apply_cli_runtime_overrides(&mut merged_config, cli);
    }
...
async fn run_one_shot(config: &Config, model: &str, prompt: &str) -> Result<()> {
    ...
    let reasoning_effort = route
        .reasoning_effort
        .map(|effort| effort.as_setting().to_string())
        .or_else(|| config.reasoning_effort().map(str::to_string));
